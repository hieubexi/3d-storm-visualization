...
class Record(object):
    ...
    def record_shift(self, corr, gl_shift, l_heads, local_shift, case):
        ...
        self.new_shifts = pd.concat([self.new_shifts, pd.DataFrame(new_shift_record)], ignore_index=False)
    ...
    def add_uids(self, current_objects):
        ...
        if len(self.new_shifts) > 0:
            ...
            self.shifts = pd.concat([self.shifts, pd.DataFrame(self.new_shifts)], ignore_index=False)
            self.new_shifts = pd.DataFrame()
